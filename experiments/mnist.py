...
def pretext_task_sensitivity_saliency(
    random_seed: int = 1,
    batch_size: int = 300,
    n_runs: int = 5,
    dim_latent: int = 4,
    ...
    # Create saving directory
    save_dir = Path.cwd() / "results/mnist/pretext_saliency"
    load_dir = Path.cwd() / "results/mnist/pretext"
    if not save_dir.exists():
        logging.info(f"Creating saving directory {save_dir}")
        os.makedirs(save_dir)
    ...
    for run in range(n_runs):
        ...
        for pretext in pretext_list:
            ...
            logging.info("Computing feature importance")
            baseline_image = torch.zeros((1, 1, 28, 28), device=device)
            grad_saliency = Saliency(encoder)
            feature_importance.append(
                np.abs(
                    np.expand_dims(
                        attribute_auxiliary(
                            encoder, test_loader, device, grad_saliency, baseline_image
                        ),
                        0,
                    )
                )
...
def disvae_feature_importance_saliency(
    random_seed: int = 1,
    batch_size: int = 300,
    n_plots: int = 20,
    n_runs: int = 5,
    ...
    # Create saving directory
    save_dir = Path.cwd() / "results/mnist/vae_saliency"
    load_dir = Path.cwd() / "results/mnist/vae"
    if not save_dir.exists():
        logging.info(f"Creating saving directory {save_dir}")
        os.makedirs(save_dir)
    ...
    for beta, loss, run in itertools.product(
        ...
        # Compute test-set saliency and associated metrics
        baseline_image = torch.zeros((1, 1, W, W), device=device)
        grad_saliency = Saliency(encoder.mu)
        logging.info(f"Defined grads")
        attributions = attribute_individual_dim_saliency(
            encoder.mu, dim_latent, test_loader, device, grad_saliency, baseline_image
        )
        logging.info(f"Attributes computed")
        metrics = compute_metrics(attributions, metric_list)
        logging.info(f"Metrics computed")
...
if __name__ == "__main__":
    ...
    elif args.name == "disvae_saliency":
        disvae_feature_importance_saliency(
            n_runs=args.n_runs, batch_size=args.batch_size, random_seed=args.random_seed
        )
    ...
    elif args.name == "pretext_saliency":
        pretext_task_sensitivity_saliency(
            n_runs=args.n_runs, batch_size=args.batch_size, random_seed=args.random_seed
        )
    ...
    elif args.name == "consistency_features_with_deeplift":
        consistency_feature_importance_with_deeplift(
            batch_size=args.batch_size, random_seed=args.random_seed
        )
