...
from lfxai.explanations.features import attribute_auxiliary, attribute_individual_dim, attribute_individual_dim_saliency
...
def pretext_task_sensitivity(
    ...
    save_dir = Path.cwd() / f"results/mnist/pretext"
    load_dir = Path.cwd() / f"results/mnist/pretext"
    if not save_dir.exists():
    ...
    for run in range(n_runs):
        ...
        for pretext in pretext_list:
            ...
            if method == 'integrated_gradients':
                ...
                gradshap = Saliency(encoder)
            else:
                gradshap = GradientShap(encoder)
            logging.info(f"Using method: {gradshap}")
        ...
        logging.info(f"Now fitting {name}")
        classifier_path = load_dir / (name + ".pt")
        # Fit a model if it does not exist yet
        ...
        fig_features.savefig(save_dir / f"{method}_saliency_maps_run{run}.pdf")
        ...
        fig_examples.savefig(save_dir / f"{method}_top_examples_run{run}.pdf")
        plt.close(fig_features)
    ...
    with open(save_dir / f"{method}_tables.tex", "w") as f:
...
def disvae_feature_importance(
    ...
    csv_path = save_dir / f"metrics_{method}.csv"
    ...
    for beta, loss, run in itertools.product(
        ...
        logging.info("Computing feature importance")
        if method == 'integrated_gradients':
            gradshap = IntegratedGradients(encoder.mu)
            attributions = attribute_individual_dim(
                encoder.mu, dim_latent, test_loader, device, gradshap, baseline_image
            )
        elif method == 'saliency':
            gradshap = Saliency(encoder.mu)
            attributions = attribute_individual_dim_saliency(
                encoder.mu, dim_latent, test_loader, device, gradshap, baseline_image
            )
        ...
        fig.savefig(save_dir / f"{name}_{method}.pdf")
        plt.close(fig)

    fig = vae_box_plots(pd.read_csv(csv_path), metric_names)
    fig.savefig(save_dir / f"metric_box_plots_{method}.pdf")
    plt.close(fig)
